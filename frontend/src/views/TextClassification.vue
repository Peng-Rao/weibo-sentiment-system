...
const handleSubmit = async () => {
    ...
    try {
        let responseData = await getPrediction('/predict', {text: inputData.value});
        let probabilities = responseData['probabilities'];
        chartData.value.series = [probabilities['positive'], probabilities['negative']];
...
<template>
    <default-layout>
        <label for="message" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">推文</label>
        <textarea
            rows="6"
        ...
        <div class="mt-4 grid grid-cols-12 gap-4 md:mt-6 md:gap-6 2xl:mt-7.5 2xl:gap-7.5">
            ...
            >
                <div class="mb-3 justify-between gap-4 sm:flex">
                    <div>
                        <h4 class="text-xl font-bold text-black dark:text-white">情感分类</h4>
                    </div>
                </div>
                <div class="mb-2">
